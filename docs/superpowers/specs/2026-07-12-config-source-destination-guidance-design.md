...
`~` / `%USERPROFILE%` 是用户主目录:Claude Code 在所有平台都把配置放在 `<home>/.claude` 下,
仅主目录写法随 shell 不同:

- macOS / Linux(以及 Windows 上的 WSL / Git Bash):`~/.claude/plugins/claude-hud/config.json`
- Windows 原生 CMD / PowerShell:`%USERPROFILE%\.claude\plugins\claude-hud\config.json`

因此引导 UI **同时列出两种平台路径**,不做「浏览器系统 = 跑 CC 的系统」的误判。
...
导入与导出弹窗都需要展示两条平台路径(macOS / Linux 与 Windows)。路径与标签集中在 `src/lib/constants.ts` 的 `CONFIG_PATHS` 常量里(`{ os, path }[]`),两个组件 `v-for` 渲染,避免文案手写出错。展示样式:小号平台标签 + 等宽路径框(`var(--bg-deep)` 背景、可选中、横向滚动不撑破弹窗)。导出侧每条路径右侧带「复制路径」按钮;导入侧仅展示(读取来源,无需复制)。样式在两个组件内各自 scoped(文案量小,不强制抽公共组件)。
